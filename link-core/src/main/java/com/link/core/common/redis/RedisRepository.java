package com.link.core.common.redis;
...
import com.link.core.common.redis.config.CacheManagerProperties;
import com.link.core.common.redis.config.RedisGenericFastJsonRedisSerializer;
...
public class RedisRepository {
    ...
    public void set(String key, Object value) {
        set(key, value, cacheManagerProperties.getKeyExpire());
    }
    ...
    public boolean putIfPresent(String key, Object value) {
        return putIfPresent(key, value, cacheManagerProperties.getKeyExpire());
    }
    ...
    public boolean putIfPresent(String key, Object value, long expireSeconds) {
        ...
        RedisGenericFastJsonRedisSerializer valueSerializer = (RedisGenericFastJsonRedisSerializer) redisTemplate.getValueSerializer();
    ...
    public void putHashValue(String key, String hashKey, Object hashValue) {
        putHashValue(key, hashKey, hashValue, cacheManagerProperties.getKeyExpire());
    }
     ...
     */
    public String getSerialNumber(String prefix, int length) {
        return getSerialNumber(prefix, length, DEFAULT_PADDING);
    }

    /**
     * 流水号，不会重置，返回指定长度的字符，可配置填充字符
     *
     * @author Link
     * @since 2024/12/11 15:20
     * @param prefix prefix
     ...
     * @return java.lang.String
     */
    public String getSerialNumber(String prefix, int length, char padding) {
        long serialNumber = incrBy("serial:" + prefix, 1L);
        String serialNumberStr = String.valueOf(serialNumber);
        ValidatorUtils.isTrue(serialNumberStr.length() <= length, BaseResponseCode.SERIAL_NUMBER_ALREADY_TOO_LONG);
        return StrUtil.fillBefore(serialNumberStr, padding, length);
    }
     ...
     *
     * @author Link
     * @since 2024/12/11 15:10
     * @param prefix 前缀
     * @param nowNum 当前值
     */
    public void initSerialNumber(String prefix, int nowNum) {
        String key = "serial:" + prefix;
        if (exists(key)) {
            int redisNum = getInteger(key);
            if (nowNum > redisNum) {
                setNoExpire(key, nowNum);
            }
        } else {
            setNoExpire(key, nowNum);
        }
    ...
    /**
     * 流水号，每天重置，返回指定长度的字符，填充字符默认0,可直接拼接指定的日期格式在前缀后<br>
     * 如prefix:OR length:5 dateFormat:yyyyMMdd<br>
     * 返回： 2024121100001
     *
     * @author Link
     * @since 2024/12/11 15:05
     * @param prefix 前缀
    ...
    public String getSerialDayPeriodNumber(String prefix, SimpleDateFormat dateFormat, int length) {
        return dateFormat.format(new Date()) + getSerialPeriodNumber(prefix, length, DEFAULT_PADDING);
    }

    /**
     * 流水号，每天重置，返回指定长度的字符，填充字符默认0<br>
     * 如prefix:OR length:5<br>
     * 返回： 00001
     *
     * @author Link
     * @since 2024/12/11 15:00
     * @param prefix 前缀
     * @param length 流水号长度
     * @return java.lang.String
     */
    public String getSerialPeriodNumber(String prefix, int length) {
        return getSerialPeriodNumber(prefix, length, DEFAULT_PADDING);
    }

    /**
     * 流水号，每天重置，返回指定长度的字符，可配置填充字符<br>
     * 如prefix:OR length:5 padding:*<br>
     * 返回： ****1
     *
     * @author Link
     * @since 2024/12/11 14:54
     * @param prefix 前缀
     * @param length 流水号长度
     * @param padding 填充字符
     * @return java.lang.String
     */
    public String getSerialPeriodNumber(String prefix, int length, char padding) {
        long serialNumber = incrBy("serialPeriod:" + prefix, 1, getTodaySecondsRemaining());
        String serialNumberStr = String.valueOf(serialNumber);
     ...
     *
     * @author Link
     * @since 2024/12/11 14:27
     * @param prefix 前缀
     * @param nowNum 当前值
     */
    public void initSerialPeriodNumber(String prefix, int nowNum) {
        String key = "serialPeriod:" + prefix;
        if (exists(key)) {
            int redisNum = getInteger(key);
            if (nowNum > redisNum) {
                set(key, nowNum, getTodaySecondsRemaining());
            }
        } else {
            set(key, nowNum, getTodaySecondsRemaining());
        }
     ...
     * @return long
     */
    private long getTodaySecondsRemaining() {
        ...
        return (midnight.getTime().getTime() - currentDate) / 1000;
    }
}
